# 时尚搭配师 Agent（Fashion Stylist Agent）

本项目将原有的“智能衣橱”升级为一个面向产业落地的智能体：**时尚搭配师 Agent**，即用户的私人 AI 时尚顾问。该 Agent 具备多模态感知、可解释决策、可视化执行与在线学习闭环，面向服装零售与个人消费者提供可执行的搭配与虚拟试穿解决方案。

## 📋 项目定位（一句话）

将衣物识别、风格推理、环境感知与视觉生成能力组合成“可执行”的产业级 Agent，帮助零售与用户提升成交率、降低退货并提升衣物利用率。

### 核心能力（Agent 视角）

- 感知：图像识别与多模态输入（衣物照片、用户肖像、天气、场景标签），自动抽取品类、风格、材质、色彩等属性。
- 决策：基于规则+模型的多目标推理，输出 3-5 套可解释的搭配候选及排序理由（reason 字段）。
- 执行：将搭配方案转化为高质量试穿效果图与导购文案，支持批量生成（多肤色/多体型）。
- 学习：基于显式反馈（收藏/喜欢/购买）与隐式信号（停留、点击、复看）进行在线重排与离线模型更新，形成闭环优化。

（以下为系统原有功能，在 Agent 框架下作为子能力保留）
...
### **定位与价值（面向深圳/大湾区）**
深圳与大湾区拥有完整的时尚产业链与成熟的数字产业基础：设计、制造、批发、零售、跨境电商、直播带货集成在地理与产业网络上。这为一个产学研一体化的“时尚搭配师 Agent”提供了天然的落地土壤。

本 Agent 以降低退货率、提升门店与电商转化、减少摄影/模特成本、提升用户衣物复用率为商业目标，形成“数据—决策—生产”闭环，反哺设计与供应链（C2M）。

### **场景一：智慧零售体验升级（To B）**
- 部署场景：南油、白马（批发市场）与中高端买手店的智能试衣镜、导购平板。
- Agent 行为：顾客选衣 → Agent 实时感知衣物与顾客形象 → 生成虚拟试穿与 3-5 套可解释搭配 → 显示材质/护理/造型建议 → 记录偏好与成交数据。
- 关键指标：试穿转化率 ↑、退货率 ↓、客单价 ↑、库存周转率提升。

### **场景二：批量内容生产与直播导购（To B）**
- 部署场景：龙岗跨境电商园、直播间素材中心。
- Agent 行为：对海量 SKU 批量生成多体型/多肤色模特试穿图与短视频片段；直播中即时替换主播/模特试穿效果，支持观众交互的“实时试穿”功能。
- 关键指标：素材生产成本 ↓、上新速度 ↑、直播转化率 ↑。

### **场景三：服务大湾区居民的品质生活（To C）**
- 部署场景：移动端 App / 小程序，面向港澳珠三地通勤白领与家庭用户。
- Agent 行为：基于实时天气（深圳/香港/广州差异）、行程日历与衣橱，作为“每日穿搭助理”推送早晨搭配建议，并在购物决策中提供可解释理由与搭配套装推荐。
- 社会价值：减少冲动消费与闲置衣物，推动可持续消费意识。

### **数据反哺闭环（产业价值核心）**
- 端到端：顾客偏好与搭配行为 → 聚合分析（门店/电商）→ 设计与采购优化（C2M）→ 上新更精准 → 降低库存与利润提升。

## **落地路线（12 周可交付计划）**

1. 第 1-2 周：项目叙事与指标对齐
   - 完成答辩版 README 与 PPT 文案，明确 KPI（转化、退货、采纳率等）。
2. 第 3-4 周：可解释推荐（MVP）
   - 在后端添加 `reason` 输出、接口改造与前端展示。
   - 指标：推荐被采纳率（收藏/试穿点击率）基线测量。
3. 第 5-6 周：反馈埋点与轻量在线学习
   - 增加 `/feedback` 接口、数据库存储、并实现 `update_user_pref()` 的简单规则重排。
4. 第 7-8 周：批量试穿与多体型/肤色素材生产
   - 实现 `/batch-tryon` 批量导出接口并优化 `image_storage`，形成素材生产流水线。
5. 第 9-10 周：To B 演示与集成（试衣镜或直播脚本）
   - 在合作门店或内部 demo 环境演示在线试衣流程、直播中实时替换试穿素材。
6. 第 11-12 周：评估、优化与答辩材料整理
   - 完整报表：转化、退货、素材成本及可解释性用户调研结果；完善 PPT 与答辩演示。

## **验收指标（建议）**
- 推荐采纳率（点击试穿 / 展示）
- 收藏率与购买转化率提升
- 退货率下降（以搭配不符为因的退货）
- 每 100 SKU 的试穿素材生产时间/成本
- 用户对 `reason` 的满意度（小规模问卷采样）

---

如果你确认要我继续，我接下来可以：
1) 按清单先实现后端的 `reason` 输出与 `/feedback` 接口（提交补丁），
2) 然后把前端修改为展示 `reason` 并触发反馈事件，
3) 最后添加 Alembic 迁移和基础测试。
